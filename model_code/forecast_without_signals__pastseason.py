...
if __name__ == "__main__":
    ...
    def compute_forecasts(location, time, peak_value=None, peak_time=None):
        ...
        theta, posterior_samples,yhats = model( cases=None
                                                        , ili_n            = None                                       #<-- NO DATA SO SET TO NONE
                                                        , ili_i            = None                                       #<-- NO DATA SO SET TO NONE
                                                        , hosps            = season_time_augmented.to_numpy()[jnp.newaxis,...]
                                                        , X                = jnp.zeros_like(X)                          #<-- NO DATA SO SET TO ZEROS
                                                        , N                = population_size
                                                        , VE               = jnp.full_like(ve,0.50)                     #<-- NO DATA SO ASSUME 50/50
                                                        , location         = location)
        ...
        weekly_forecast_data.to_csv("./forecasts/without_signals/weekly_forecasts__{:s}__{:s}.csv".format(location,thisweek))
        peak_forecast_time_data.to_csv("./forecasts/without_signals/peak_forecasts_time__{:s}__{:s}.csv".format(location,thisweek))
        peak_forecast_intensity_data.to_csv("./forecasts/without_signals/peak_forecasts_intensity__{:s}__{:s}.csv".format(location,thisweek))

        all_forecasts = pd.concat([weekly_forecast_data,peak_forecast_time_data, peak_forecast_intensity_data])
        all_forecasts.to_csv("./forecasts/without_signals/all_forecasts__{:s}__{:s}.csv".format(location,thisweek))

        pickle.dump(posterior_samples, open("./time_dep_transmission_rate/without_signals_posterior_samples.pkl","wb"))
